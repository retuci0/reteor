...
import static net.retucio.reteor.utils.StrongholdPosition.getDistance;
import static net.retucio.reteor.utils.StrongholdPosition.triangulatePosition;
...
public class NbtHandler {
    ...
    public static double[] read() {
        IThrowData throwData = (IThrowData) MinecraftClient.getInstance().player;
        NbtList nbtList = throwData.getThrowData().getList("strongholdfinder.previousThrowData", NbtType.DOUBLE);

        return new double[] {nbtList.getDouble(0),
                            nbtList.getDouble(1),
                            nbtList.getDouble(2),
                            nbtList.getDouble(3)
        };
